...
# generating binary mask, based on edges generated by Canny algorithm
...
def classify(images: List[np.ndarray], verbose: bool = False) -> Tuple[List[np.ndarray], np.ndarray]:
    polygon_index = 0
    masks = []
    objects, polygon = [], None
    max_white = 0
    ...
    for i, image in enumerate(images):
        ...
        dominant = get_dominant_color(processable_image)  # get dominant color
    ...
    objects.sort(key=lambda x: -1 * sum([sum(line) for line in x]))
